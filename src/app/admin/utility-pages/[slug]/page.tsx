...
function Preview({ heading, body, label }: { heading: string; body: string; label: string }) {
  return <section className="rounded-card border border-deep-navy/10 bg-white p-6 shadow-card">
    ...
    <article className="prose prose-lg mt-5 max-w-none text-charcoal/80">
      ...
      {body ? body.split(/\n{2,}/).map((paragraph, index) => <p key={index}>{paragraph}</p>) : <p className="text-charcoal/60">No body copy saved.</p>}
    </article>
  </section>;
}
...
export default async function UtilityPageAdminPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <AdminShell role={membership.role} title={definition.label} intro="This fixed utility page is Administrator-only. Draft changes stay private until they complete the existing approval and publishing workflow.">
    ...
    <section className="mt-6 rounded-card bg-white p-6 shadow-card">
      ...
      <form action={savePageContentDraft} className="mt-5 space-y-4">
        ...
        <label className="block text-sm font-semibold">Body<textarea name="body" defaultValue={draft ? draftBody : published?.body ?? ""} rows={8} className="mt-1 w-full rounded border p-2" /></label>
        <button className="button-primary" type="submit" disabled={draft?.status === CmsDraftStatus.WAITING_FOR_APPROVAL}>Save for review</button>
      ...
      {draft && <div className="mt-5 flex flex-wrap gap-3">
        {(draft.status === CmsDraftStatus.DRAFT || draft.status === CmsDraftStatus.RETURNED_FOR_CHANGES) && <form action={submitCmsDraft}><input type="hidden" name="draftId" value={draft.id} /><button className="button-secondary" type="submit">{draft.status === CmsDraftStatus.DRAFT ? "Submit for approval" : "Resubmit for approval"}</button></form>}
        {(draft.status === CmsDraftStatus.DRAFT || draft.status === CmsDraftStatus.RETURNED_FOR_CHANGES || draft.status === CmsDraftStatus.WAITING_FOR_APPROVAL) && <form action={withdrawCmsDraft}><input type="hidden" name="draftId" value={draft.id} /><button className="rounded-lg border border-swcu-red/30 px-4 py-2 font-semibold text-swcu-red" type="submit">Withdraw draft</button></form>}
