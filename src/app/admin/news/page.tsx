...
export default async function NewsAdminPage() {
  ...
  return (
    <main className="min-h-screen bg-soft-blue-grey"><div className="site-container py-12"><a href="/admin" className="text-sm font-semibold text-swcu-blue">← Dashboard</a><h1 className="mt-8 font-heading text-4xl font-bold text-deep-navy">News &amp; Notices</h1><AdminWorkflowNotice editor={editor} />
      ...
      <ul className="mt-8 space-y-3">{news.map((item) => <li key={item.id} className="rounded border bg-white p-4"><form action={saveNewsNotice} className="grid gap-2"><input type="hidden" name="id" value={item.id} /><input name="title" defaultValue={item.title} required className="rounded border p-2" /><textarea name="summary" defaultValue={item.summary} required className="rounded border p-2" /><input name="publishedAt" type="datetime-local" defaultValue={formatFijiDateTime(item.publishedAt)} className="rounded border p-2" /><label className="text-sm"><input name="isPublished" type="checkbox" defaultChecked={item.isPublished} /> {editor ? "Ready for approval" : "Published"}</label><button className="w-fit text-sm font-semibold text-swcu-blue">{editor ? "Save Draft" : "Publish changes"}</button></form><form action={removeNewsNotice} className="mt-2"><input type="hidden" name="id" value={item.id} /><button className="text-sm font-semibold text-swcu-red">{editor ? "Propose unpublish" : "Unpublish"}</button></form></li>)}</ul>
    </div></main>
  );
}