  ...
  return <AdminShell role={membership.role} title="Waiting for approval" intro="Review what will appear on the public site before you publish it.">
    {enriched.length === 0 ? <div className="mt-8 rounded-card border border-dashed border-deep-navy/20 bg-white p-10 text-center"><h2 className="font-heading text-xl font-bold text-deep-navy">Nothing is waiting</h2><p className="mt-2 text-charcoal/65">New submissions will appear here for review.</p></div> : <div className="mt-8 space-y-5">{enriched.map(({ draft, current, payload, stale, title }) => <article key={draft.id} className="rounded-card border border-deep-navy/10 bg-white p-6 shadow-card">
      ...
      {draft.mediaAssetId && <p className="mt-4 rounded-lg bg-ocean-teal/10 px-4 py-3 text-sm text-deep-navy">A new file is included with this submission. Check it before publishing.</p>}
        <div className="mt-5 flex flex-wrap items-center gap-2">{(!draft.targetId || current) && <AdminActionForm action={publishCmsDraft} className="contents" successMessage="Draft published."><input type="hidden" name="draftId" value={draft.id}/>{stale && <label className="flex items-center gap-2 text-sm"><input type="checkbox" name="confirmPublishedChange" value="true" required/> I reviewed the changed published version</label>}<AdminSubmitButton pendingLabel="Publishing…" className="button-primary">Publish</AdminSubmitButton></AdminActionForm>}<AdminActionForm action={returnCmsDraft} className="flex gap-2" successMessage="Draft returned for changes."><input type="hidden" name="draftId" value={draft.id}/><input name="note" aria-label="Note for returned draft" placeholder="Optional note" className="rounded-lg border border-deep-navy/15 px-3 text-sm"/><AdminSubmitButton pendingLabel="Returning…" className="button-secondary">Return for changes</AdminSubmitButton></AdminActionForm><AdminActionForm action={archiveCmsDraft} className="contents" successMessage="Draft archived."><input type="hidden" name="draftId" value={draft.id}/><ConfirmSubmitButton label="Archive" message="Archive this approval draft?" className="rounded-lg border border-swcu-red/30 px-4 py-2 font-semibold text-swcu-red"/></AdminActionForm></div>
    </article>)}</div>}
  </AdminShell>;
}