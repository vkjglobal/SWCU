...
export default async function FormsAdminPage() {
  ...
  return <main className="min-h-screen bg-soft-blue-grey"><div className="site-container py-12">
    ...
    <ul className="mt-8 max-w-3xl space-y-3">{forms.map((form) => <li key={form.id} className="rounded border bg-white p-4">
       ...
       <div className="mt-3 flex gap-3"><AdminActionForm action={replaceFormDocument} encType="multipart/form-data" className="flex items-center gap-2" successMessage="PDF replaced successfully."><input type="hidden" name="id" value={form.id}/><input type="hidden" name="revision" value={revisionFor(form.id)}/><input name="file" type="file" accept="application/pdf,.pdf" required className="max-w-44 text-xs"/><AdminSubmitButton pendingLabel="Replacing…" className="font-semibold text-ocean-teal">{editor ? "Propose replacement" : "Replace PDF"}</AdminSubmitButton></AdminActionForm></div>
    </li>)}</ul>
  </div></main>;
}