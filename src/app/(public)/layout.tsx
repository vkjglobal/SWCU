...
export default async function PublicLayout({
  ...
  const [contact, pages] = await Promise.all([getPublicContactSettings(tenant), getPublishedPageContent(tenant, ["PRIVACY", "TERMS_OF_USE", "ACCESSIBILITY"])]);
  const published = { privacy: pages.some((p) => p.slot === "PRIVACY"), terms: pages.some((p) => p.slot === "TERMS_OF_USE"), accessibility: pages.some((p) => p.slot === "ACCESSIBILITY") };
