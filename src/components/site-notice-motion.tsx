...
export function SiteNoticeMotion({ notice }: { notice: Notice }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const messageWidth = messageRef.current?.getBoundingClientRect().width ?? 0;
      const shouldMove = Boolean(viewport && messageWidth > viewport.clientWidth - 8);
      const loopDistance = messageWidth + 48;
      setDistance(loopDistance);
      setDuration(Math.max(8, loopDistance / 60));
  ...
  return <aside className="border-b border-swcu-blue/15 bg-soft-blue-grey" aria-label="Site notice">
    <div className="site-container flex items-center gap-3 py-2 text-sm" onMouseEnter={() => setHoveredOrFocused(true)} onMouseLeave={() => setHoveredOrFocused(false)} onFocusCapture={() => setHoveredOrFocused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setHoveredOrFocused(false); }}>
      <div ref={viewportRef} className="site-notice-viewport min-w-0 flex-1 overflow-hidden">
        <span ref={messageRef} className={moving ? "sr-only" : "inline-block whitespace-nowrap font-medium text-deep-navy"}>{notice.message}</span>
        {moving && <span aria-hidden="true" style={{ "--notice-distance": `${distance}px`, "--notice-duration": `${duration}s` } as React.CSSProperties} className={`site-notice-track inline-flex w-max whitespace-nowrap font-medium text-deep-navy${animationPaused ? " is-paused" : ""}`}><span>{notice.message}</span><span className="ml-12">{notice.message}</span></span>}
      </div>
