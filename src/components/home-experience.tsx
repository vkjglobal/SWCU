...
import {
  ArrowDownRight, ArrowLeft, ArrowRight, Calculator, ChevronDown,
  FileText, HandCoins, HeartHandshake, Landmark, Mail, MapPin,
  Phone, PiggyBank, UserRound, WalletCards,
  Sunset,
} from "lucide-react";
...
export interface NewsNotice { title: string; date: string; excerpt?: string; href?: string; }
export interface FAQ { question: string; answer: string; }
export interface ContactSettings {
  organisationName: string; streetAddress: string; postalAddress: string; telephone: string; publicEmail: string;
}
export interface MemberAppSettings { enabled: boolean; label: string; href: string; }
...
function Icon({ name }: { name?: string }) {
  ...
  const IconComponent = icons[name?.trim().toLowerCase() as keyof typeof icons] ?? PiggyBank;
  return <IconComponent {...props} />;
}

export function HomeExperience({ heroSlides = [], highlights = [], services = [], forms = [], news = [], faqs = [], contact, memberApp = { enabled: true, label: "Member App — Coming Soon", href: "/member-login" } }: HomeExperienceProps) {
  const reduce = useReducedMotion();
  ...
  useEffect(() => {
    if (reduce || carouselPaused || slides.length < 2) return;
    timer.current = setTimeout(next, Math.max(7000, manualPauseUntil.current - Date.now()));
    return () => { if (timer.current) clearTimeout(timer.current); };
  }, [next, slide, reduce, carouselPaused, slides.length]);
  ...
  return (
    <div className="overflow-hidden bg-white">
        ...
        <div className="site-container grid min-h-[640px] items-center gap-10 py-12 md:py-20 lg:grid-cols-[.92fr_1.08fr] lg:py-24">
          <Reveal immediate className="relative z-10">
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              ...
              <Link href={memberApp.href} className="home-link button-secondary">Member Login</Link>
            </div>
      ...
      <section id="trust-strip" className="border-y border-deep-navy/10 bg-white"><div className="site-container grid divide-y divide-deep-navy/10 py-2 sm:grid-cols-3 sm:divide-x sm:divide-y-0">{highlights.length > 0 ? highlights.slice(0, 3).map((h) => <Reveal key={h.value} className="flex items-baseline gap-3 px-4 py-5 sm:first:pl-0"><strong className="display-heading text-2xl text-deep-navy md:text-3xl">{h.value}</strong>{h.label === "Members" && <span className="text-sm font-medium text-charcoal/65">{h.label}</span>}</Reveal>) : <div className="col-span-full flex items-center gap-3 py-5 text-sm text-charcoal/65"><span className="size-2 rounded-full bg-swcu-blue" />Homepage highlights will appear here when available.</div>}</div></section>

      <section id="why-swcu" className="bg-soft-blue-grey py-20 md:py-28"><div className="site-container"><Reveal><h2 className="display-heading max-w-xl text-4xl font-bold leading-tight text-deep-navy md:text-5xl">A Credit Union Built Around Its Members</h2><div className="story-line mt-7" /></Reveal><div className="mt-12 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[["Build Your Savings", "Develop regular savings for today and the future."], ["Access Member Loans", "Financial assistance for approved needs."], ["Member Protection", "Member schemes designed to support members and their families."], ["Personal Member Service", "A credit union focused on its members."]].map(([title, description], i) => <Reveal key={title} className={`flex min-h-[190px] flex-col border-l-4 p-5 ${i === 0 ? "border-swcu-red bg-white" : "border-swcu-blue/25 bg-white/60"}`}><span className="text-sm font-bold text-swcu-blue">0{i + 1}</span><h3 className="display-heading mt-8 text-xl font-semibold text-deep-navy">{title}</h3><p className="mt-3 text-sm leading-6 text-charcoal/70">{description}</p></Reveal>)}</div></div></section>

      <section id="services" className="bg-white py-20 md:py-28"><div className="site-container"><Reveal><h2 className="display-heading max-w-2xl text-4xl font-bold text-deep-navy md:text-5xl">Services for Every Stage of Membership</h2></Reveal>{serviceIcons.length > 0 ? <div className="service-scroller mt-12 flex snap-x gap-4 overflow-x-auto pb-5 md:grid md:grid-cols-4 md:overflow-visible">{serviceIcons.map((service, i) => <Reveal key={service.title} className="min-w-[82vw] snap-start sm:min-w-[340px] md:min-w-0"><Link href={service.href || "#services"} className="home-link group flex min-h-[280px] flex-col justify-between rounded-2xl border border-deep-navy/10 bg-soft-blue-grey p-6"><span className={`grid size-14 place-items-center rounded-full ${i % 2 ? "bg-swcu-blue text-white" : "bg-white text-ocean-teal"}`}><Icon name={service.icon} /></span><div><h3 className="display-heading text-2xl font-semibold text-deep-navy">{service.title}</h3><p className="mt-3 text-charcoal/70">{service.description}</p></div><span className="flex items-center gap-2 text-sm font-bold text-swcu-blue">Explore <ArrowRight size={16} className="transition-transform group-hover:translate-x-1" /></span></Link></Reveal>)}</div> : <div className="mt-12 border-l-4 border-swcu-blue/30 bg-soft-blue-grey p-6 text-charcoal/70">Services will appear here when available.</div>}</div></section>

      <section id="loan-calculator" className="bg-deep-navy py-20 text-white md:py-28"><div className="site-container grid gap-12 lg:grid-cols-[.85fr_1.15fr] lg:items-center"><Reveal><h2 className="display-heading text-4xl font-bold md:text-5xl">Plan Your Loan</h2><p className="mt-6 max-w-md text-lg leading-8 text-white/70">See an estimated repayment before you apply.</p></Reveal><Reveal className="rounded-[1.5rem] bg-white p-6 text-charcoal shadow-[0_24px_50px_rgba(0,0,0,.16)] md:p-8"><div className="flex items-center justify-between"><p className="section-kicker">Loan calculator</p><Calculator className="text-swcu-blue" /></div><div className="mt-7 grid gap-5 sm:grid-cols-2"><label className="grid gap-2 text-sm font-semibold">Loan Amount<input inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value.replace(/[^0-9.]/g, ""))} placeholder="Enter amount" className="min-h-12 rounded-xl border border-deep-navy/15 px-4 outline-none focus:border-swcu-blue" /></label><label className="grid gap-2 text-sm font-semibold">Loan Term<input inputMode="numeric" value={term} onChange={(e) => setTerm(e.target.value.replace(/[^0-9]/g, ""))} placeholder="Enter months" className="min-h-12 rounded-xl border border-deep-navy/15 px-4 outline-none focus:border-swcu-blue" /></label></div><label className="mt-5 grid gap-2 text-sm font-semibold">Repayment Frequency / salary/pay period<select value={frequency} onChange={(e) => setFrequency(e.target.value)} className="min-h-12 rounded-xl border border-deep-navy/15 bg-white px-4"><option>Salary / pay period</option><option>Weekly</option><option>Fortnightly</option><option>Monthly</option></select></label><div className="mt-7 rounded-xl bg-soft-blue-grey p-5"><p className="font-heading text-xl font-bold text-deep-navy">Calculator status: Awaiting SWCU configuration</p><p className="mt-2 text-sm leading-6 text-charcoal/70">Repayment figures are estimates only. Actual repayments, terms and loan approval are subject to SWCU requirements and approval.</p></div><button type="button" disabled aria-label="Calculator unavailable until configured" className="mt-5 min-h-12 w-full cursor-not-allowed rounded-xl bg-deep-navy/10 font-bold text-deep-navy/45">Calculate</button></Reveal></div></section>

      <section id="become-a-member" className="bg-soft-blue-grey py-20 md:py-28"><div className="site-container"><Reveal><h2 className="display-heading text-4xl font-bold text-deep-navy md:text-5xl">Become an SWCU Member</h2></Reveal><div className="relative mt-14 grid gap-8 md:grid-cols-3 md:gap-0">{[["01", "Check Your Eligibility"], ["02", "Complete Your Application"], ["03", "Start Saving"]].map(([number, title], i) => <Reveal key={number} className="relative flex gap-5 md:block md:pr-10"><div className="relative z-10 grid size-14 shrink-0 place-items-center rounded-full bg-swcu-blue font-heading font-bold text-white ring-8 ring-soft-blue-grey">{number}</div><div className="pt-1 md:mt-7"><h3 className="display-heading text-xl font-semibold text-deep-navy">{title}</h3></div>{i < 2 && <span className="absolute left-7 top-14 h-[calc(100%+2rem)] w-px bg-swcu-blue/25 md:left-7 md:top-7 md:h-px md:w-[calc(100%-1rem)]" />}</Reveal>)}</div><div className="mt-12 flex flex-wrap gap-3"><Link href="/membership-services#membership" className="button-primary">Membership Details <ArrowRight size={17} /></Link>{membershipForm && <a href={membershipForm.href} className="button-secondary">Download Membership Form <FileText size={17} /></a>}</div></div></section>

       {memberApp.enabled && <section id="member-app" className="app-grid relative overflow-hidden bg-deep-navy py-20 text-white md:py-28"><div className="absolute -right-48 top-10 size-[30rem] rounded-full border border-white/10" /><div className="site-container grid items-center gap-14 lg:grid-cols-[1fr_.9fr]"><Reveal><h2 className="display-heading max-w-xl text-5xl font-bold leading-tight">Your SWCU. Wherever You Are.</h2><div className="mt-8 flex flex-wrap gap-3">{["Balances", "Loans", "Transactions", "Statements", "Forms & Requests", "Member Notices"].map((x) => <span key={x} className="rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm text-white/80">{x}</span>)}</div><Link href={memberApp.href} className="button-secondary mt-10 border-white/30 bg-white/10 text-white hover:bg-white/15">{memberApp.label}</Link></Reveal><Reveal className="flex justify-center lg:justify-end"><div className="relative w-[min(78vw,300px)] rotate-2 rounded-[2.7rem] border-[7px] border-charcoal bg-charcoal p-2 shadow-[0_28px_70px_rgba(0,0,0,.35)]"><div className="overflow-hidden rounded-[2rem] bg-soft-blue-grey text-charcoal"><div className="bg-swcu-blue px-5 pb-8 pt-7 text-white"><p className="text-xs font-semibold text-white/70">SWCU MEMBER APP</p><p className="mt-3 font-heading text-2xl font-bold">Member App — Coming Soon</p></div><div className="grid grid-cols-2 gap-3 p-4">{["Balances", "Loans", "Transactions", "Statements"].map((x) => <div key={x} className="rounded-xl bg-white p-3 text-xs font-semibold shadow-sm">{x}<span className="mt-3 block h-2 w-10 rounded bg-swcu-blue/20" /></div>)}</div></div></div></Reveal></div></section>}

      <section id="help-resources" className="bg-white py-20 md:py-28"><div className="site-container grid gap-14 lg:grid-cols-[.9fr_1.1fr]"><Reveal><h2 className="display-heading text-4xl font-bold text-deep-navy md:text-5xl">Help & Resources</h2><div className="mt-10 border-t border-deep-navy/10">{forms.slice(0, 6).map((form) => <div key={form.title} className="flex items-center justify-between border-b border-deep-navy/10 py-5"><span className="flex items-center gap-3 font-semibold text-deep-navy"><FileText size={19} className="text-swcu-blue" />{form.title}</span>{form.href && <a href={form.href} className="text-sm font-bold text-swcu-blue">Download</a>}</div>)}{!forms.length && <p className="py-5 text-charcoal/65">Forms and documents will appear here when available.</p>}</div>{news.slice(0, 3).length > 0 && <div className="mt-12"><p className="section-kicker">Latest from SWCU</p>{news.slice(0, 3).map((item) => <article key={`${item.title}-${item.date}`} className="mt-4 border-b border-deep-navy/10 pb-4"><p className="text-xs font-bold text-swcu-red">{item.date}</p><h3 className="mt-1 font-semibold text-deep-navy">{item.title}</h3>{item.excerpt && <p className="mt-2 line-clamp-3 text-sm leading-6 text-charcoal/70">{item.excerpt}</p>}</article>)}<Link href="/forms-resources#news" className="button-secondary mt-5">View News &amp; Notices <ArrowRight size={16} /></Link></div>}</Reveal><Reveal>{faqs.length > 0 && <div className="rounded-2xl bg-soft-blue-grey p-6 md:p-8"><p className="section-kicker">How Can We Help?</p><div className="mt-5 divide-y divide-deep-navy/10">{faqs.map((item, i) => <div key={item.question}><button type="button" aria-expanded={faq === i} onClick={() => setFaq(faq === i ? null : i)} className="flex min-h-16 w-full items-center justify-between gap-4 text-left font-semibold text-deep-navy transition-colors hover:text-swcu-blue active:text-ocean-teal">{item.question}<ChevronDown size={19} className={`shrink-0 transition-transform ${faq === i ? "rotate-180" : ""}`} /></button><AnimatePresence initial={false}>{faq === i && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: reduce ? 0 : .2 }} className="overflow-hidden"><p className="pb-5 pr-8 text-charcoal/70">{item.answer}</p></motion.div>}</AnimatePresence></div>)}</div></div>}{contact && <div className={`${faqs.length > 0 ? "mt-5" : ""} grid gap-4 rounded-2xl bg-deep-navy p-6 text-white md:grid-cols-[1fr_auto] md:items-center`}><div><p className="section-kicker text-white/60">Contact SWCU</p><h3 className="display-heading mt-2 text-2xl font-semibold">{contact.organisationName}</h3><p className="mt-2 text-sm leading-6 text-white/70"><MapPin size={14} className="mr-1 inline" />{contact.streetAddress}<br />{contact.postalAddress}</p></div><div className="text-sm text-white/80"><a className="flex items-center gap-2" href={telephoneHref(contact.telephone)}><Phone size={15} />{formatTelephone(contact.telephone)}</a><a className="mt-2 flex items-center gap-2" href={`mailto:${contact.publicEmail}`}><Mail size={15} />{contact.publicEmail}</a></div></div>}</Reveal></div></section>
    </div>
  );
}