...
export function HomeExperience({ heroSlides = [], highlights = [], services = [], forms = [], news = [], faqs = [], contact, memberApp = { enabled: true, label: "Member App — Coming Soon", href: "/member-login" } }: HomeExperienceProps) {
  ...
  return (
    <div className="overflow-hidden bg-white">
      ...
      <section id="trust-strip" className="border-y border-deep-navy/10 bg-white"><div className="site-container grid divide-y divide-deep-navy/10 py-2 sm:grid-cols-3 sm:divide-x sm:divide-y-0">{highlights.length > 0 ? highlights.slice(0, 3).map((h) => <Reveal key={h.value} className="flex items-baseline gap-3 px-4 py-5 first:pl-0"><strong className="display-heading text-2xl text-deep-navy md:text-3xl">{h.value}</strong>{h.label === "Members" && <span className="text-sm font-medium text-charcoal/65">{h.label}</span>}</Reveal>) : <div className="col-span-full flex items-center gap-3 py-5 text-sm text-charcoal/65"><span className="size-2 rounded-full bg-swcu-blue" />Homepage highlights will appear here when available.</div>}</div></section>
